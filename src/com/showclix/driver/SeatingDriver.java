...
public class SeatingDriver {
	...
	/**
	 * Initializing a Driver object will create a new seating arrangement based
	 * on the number of rows and columns specified in the configuration file. By
	 * default, the number of rows is 3 and the number of columns is 11
	 */
	public void initialize() {
		// If no configuration file is found, the values 3 and 11 will be used
		// per original spec
	...
	/**
	 * This method will listen for user input, expecting the format specified in
	 * the project requirements. As input is entered, reservations will be
	 * attempted and the status of the reservation will be output back to the
	 * console/stdout
	 */
	public void listen() {
		...
		while (scanner.hasNext()) {
			...
			if (linesRead == 0) {
				...
				// otherwise, interpret the line as a number that represents a
				// group size
	...
	/**
	 * If the user passed the filename in as an argument, instead of listening
	 * for user input, this method will will read the file line by line. The
	 * first line is expected to be a space delimited list of pre-reservations
	 * while the following lines are expected to be the number of seats
	 * requested per reservation.
	 * 
	 * @param filePath
	 *            - path/file name of the input data on disk
	 */
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (linesRead == 0) {
					...
					// otherwise, interpret the line as a number that represents
					// a group size and try to find the best seating for the
					// group
	...
	/**
	 * This is a helper method that will split the input line on space
	 * characters and try to parse out the row and column of each entry. Once
	 * parsed, a pre-reservation will be made on the seat.
	 * 
	 * @param line
	 *            Input string, expected format: "R4C3 R3C9 R1C4 R8C4" etc
	...
	/**
	 * This is a helper method that will allow the driver to ask the seating
	 * arrangement to place a group in the audience. The seats will be printed
	 * or Not Available if they couldn't be placed.
	 * 
	 * @param seatingRequest
	 *            The number of people in the group that need to be seated
	 * @return True if the input data was parsed and submitted successfully,
	 *         false otherwise
	 */
